...
import vivae.arena.parts.BadFood;
 ...
 * @author HKou
 */
public class TomMultiSensor extends Sensor {

    protected Active owner;
    protected Body ownerBody;
    protected float ray_length = 50f;
    protected float ray_width = 2f;
    protected float angle = 0f;
    protected AlphaComposite opacityOfRay = AlphaComposite.getInstance(AlphaComposite.SRC_OVER, 0.2f);
    ...
    public Vector<VivaeObject> getCloseVivaes(Vector<VivaeObject> objects) {
        ...
        int d = i + 1;
        while (d < objects.size() && objects.get(d).getBody().getPosition().getX() - ownerBody.getPosition().getX() < ray_length) {
            if (objects.get(d).getBoundingCircleRadius() + ray_length > objects.get(d).getBody().getPosition().distance(ownerBody.getPosition())) {
        ...
        d = i - 1;
        while (d >= 0 && (ownerBody.getPosition().getX() - objects.get(d).getBody().getPosition().getX()) < ray_length) {
            if (objects.get(d).getBoundingCircleRadius() + ray_length > objects.get(d).getBody().getPosition().distance(ownerBody.getPosition())) {
...
//        }
    ...
    public static Vector<VivaeObject> getCloseFood(Vector<VivaeObject> objects) {
    ...
    public static Vector<VivaeObject> getCloseBadFood(Vector<VivaeObject> objects) {

        Vector<VivaeObject> closeObjects = new Vector<VivaeObject>();
        for (VivaeObject vivae : objects) {
            if (vivae instanceof BadFood) {
                closeObjects.add(vivae);
            }
        }
    ...
    public static Vector<VivaeObject> getCloseRobots(Vector<VivaeObject> objects, int sp, boolean b) {

        Vector<VivaeObject> closeObjects = new Vector<VivaeObject>();
        for (VivaeObject vivae : objects) {
            if (vivae instanceof TomBot) {


                if ((((TomBot) vivae).getSpecies() == sp) == b) {
                    closeObjects.add(vivae);
                }
            }
        }
    ...
    public Vector<VivaeObject> getCloseWalls(Vector<Fixed> walls) {
...
//        if (ray_length > yPos) {
//            closeObjects.add(walls.get(0));
//        } else if (owner.getArena().screenHeight - ray_length < yPos) {
//            closeObjects.add(walls.get(1));
//        }
//        if (ray_length > xPos) {
//            closeObjects.add(walls.get(3));
//        } else if (owner.getArena().screenWidth - ray_length < xPos) {
//            closeObjects.add(walls.get(2));
//        }

        for (VivaeObject vivaeObject : walls) {
            closeObjects.add(vivaeObject);
        }
    ...
    public TomMultiSensor(Active owner, double angle, int number, double maxDistance) {
     ...
     */
    public TomMultiSensor(Active owner, int number, double maxDistance) {
        super(owner);
        this.owner = owner;
        this.ownerBody = owner.getBody();
        this.sensorNumber = number;
